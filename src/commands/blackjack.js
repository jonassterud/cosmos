...
module.exports = {
	...
	execute(message,args) {
        ...
        message.channel.send(embed).then(msg => {
            msg.react(emos[0]).then(s => {
                msg.react(emos[1]).then (s => {
                    handleGame(msg)
                }).catch();
            }).catch();
        }).catch();
        ...
        function handleGame(mess){
            mess.awaitReactions((reaction, user) => user.id == message.author.id && emos.includes(reaction.emoji.name), {max:1,time:30000}).then(coll => {
