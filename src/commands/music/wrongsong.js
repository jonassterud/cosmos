...
module.exports = {
    ...
    execute(message) {
        ...
        ytdl.getBasicInfo(queue[message.guild.id].urls.pop(), (err, data) => {
            if(err) message.channel.send(`\:open_mouth: Removed last song from queue, <@${message.author.id}>!`);
            else message.channel.send(`\:open_mouth: Removed "*${data.title}*" from the queue, <@${message.author.id}>!`);
        });
    }
};
